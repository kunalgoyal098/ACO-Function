...
def move_robot(current_position, pheromone_map, communication_range, exploration_factor):
    # Defining  the possible moves the robot can make
    possible_moves = [(1, 0), (-1, 0), (0, 1), (0, -1)]  # down, up, right, left
    
    # Getting  the dimensions of the pheromone map
    map_height, map_width = pheromone_map.shape
    
    # Initializing  a list to store the probabilities for each move
    move_probabilities = []
    
    # Calculating the probability of each move based on local pheromone concentration
    for move in possible_moves:
        new_position = (current_position[0] + move[0], current_position[1] + move[1])
        
        # Checking if the new position is within the communication range and map boundaries
    ...
    # Choosing the next move based on the calculated probabilities
    ...
    # Updating the pheromone map with decay and evaporation
